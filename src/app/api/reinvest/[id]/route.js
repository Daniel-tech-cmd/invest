...
export const POST = async (req, { params }) => {
  ...
  try {
    const { amount, crypto: method, plan } = await req.json();
    const userId = params.id;

    const user = await User.findById(userId);

    if (!user) {
      return new Response(JSON.stringify({ error: "User not found1" }), {
        status: 404,
      });
    }
    ...
    const existingPlan = user.plans.find((pla) => pla.planName === plan);
